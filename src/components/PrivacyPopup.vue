...
<template>
    <div id="privacyPolicy" class="policyPopup" v-show="isOpen">
        <span class="closepolicy" id="closePrivacyPolicy" @click="close" aria-label="Close"
            >&times;</span>
        <div id="privacyPolicyInner" class="divWithoutStyle">
...
export default {
    ...
    computed: {
        isOpen() {
            ...
            this.loadPrivacy();
            return popupStore.privacyPolicy;
        },
        pagename() {
            const generalStore = useGeneralStore();
            return generalStore.pagename; 
        },
        domainname() { //put it here to make this change after the domainname is changed
            const generalStore = useGeneralStore();
            return generalStore.domainname; // This will now be reactive
        },
    },
    methods: {
        close() {
            const popupStore = usePopupStore();
            popupStore.closePopup("privacyPolicy");
        },
        async loadPrivacy() {
            this.loading = true;
            try {
                ...
                const response = await fetch(`/legal/privacyPolicy_${language}.html`);
